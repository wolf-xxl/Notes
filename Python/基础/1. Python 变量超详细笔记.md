...
# 在循环中使用
while (line := input("请输入内容 (输入quit退出): ")) != "quit":
    print(f"你输入了: {line}")
...
builtin_scope_demo()
print(f"全局作用域的len: {len}")  # 内置len函数仍然存在
...
# 全局变量字典
print(f"全局变量字典: {list(globals().keys())}")
...
print(f"原对象: {alice}")          # Person(Alice, friends=[Person(Bob, friends=[])])
print(f"浅拷贝: {alice_shallow}")  # Person(Alice, friends=[Person(Bob, friends=[])])
...
def create_locals():
    for i in range(3):
        var_name = f"local_var_{i}"
        locals()[var_name] = i * 100
    # 注意：locals()在函数中可能不会立即更新
...
# 应该使用字典：
vars_dict = {f"var_{i}": i for i in range(10)}
...
这份超详细的Python变量笔记涵盖了变量的所有重要概念，从基础定义到高级特性，以及常见的易错点和最佳实践。希望对您的Python学习有全面的帮助！